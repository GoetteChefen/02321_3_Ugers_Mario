...
void PrintSprite(u8 *frame, u32 width, u32 height, u32 stride, u8 *sprite, u32 xcoi, u32 ycoi, u32 spriteWidth, u32 spriteHeight);



#define testArrayHeight 20
...
int main(void)
{
	...
	PrintSprite(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE, map, 0, 0, 640*4, 480);
	...
	PrintSquare_V2(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE, blackTestArray, pos_x, pos_y, testArrayWidth, testArrayHeight);
	...
	if(xcoi < height && ycoi < width) {
		...
		for(int y = 0; y < spriteHeight; y++){
			...
			for(int x = 0; x < spriteWidth; x+=4) {
				...
				frame[(xcoi*4) + (ycoi*stride) + 	 x + lineStart] = sprite[x + 2];
				frame[(xcoi*4) + (ycoi*stride) + 1 + x + lineStart] = sprite[x + 1];
				frame[(xcoi*4) + (ycoi*stride) + 2 + x + lineStart] = sprite[x];
...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
        ...
        case 'b': //Print sprite
        	PrintSprite(pFrames[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, DEMO_STRIDE, mario, 0, 0, 512*4, 480);
            break;
